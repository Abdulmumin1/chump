<script lang="ts">
    ...
    // Props definition in Svelte 5 style
    let {
        shape = "wave",
        matrix = "bayer8",
        pxSize = 2,
    ...
    let cursorState = $state<LocalCursorState>({
        x: -1,
        y: -1,
        active: false,
    });

    // Reactive state object linked to props
    ...
    function getBayer(ix: number, iy: number, currentMatrix: Matrix): number {
        if (currentMatrix === "random") return Math.random();
        if (currentMatrix === "bayer2") return B2[(iy % 2) * 2 + (ix % 2)] / 4;
        if (currentMatrix === "bayer4") return B4[(iy % 4) * 4 + (ix % 4)] / 16;
    ...
    function evalShape(nx: number, ny: number, t: number, s: AppState): number {
        ...
        if (shape === "wave") {
            ...
            if (s.flipped) {
                v = Math.tanh(boundary * 4) * 0.5 + 0.5;
            } else {
    ...
    // Canvas rendering loop in Svelte 5
    $effect(() => {
        ...
        const render = () => {
            // Read state reactively from the appState derived variable
            const s = appState;
            const elapsed = (Date.now() - startTime) * 0.001 * s.speed;
    ...
    const handleMouseMove = (e: MouseEvent & { currentTarget: EventTarget & HTMLCanvasElement }) => {
