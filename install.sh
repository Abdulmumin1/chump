...
stop_service_for_upgrade() {
    ...
    step "Stopping the local Chump service for upgrade"
    "$installed_app" stop >/dev/null 2>&1 || true
    # Versions before the shared-service architecture used a coordinator.
    "$installed_app" daemon stop >/dev/null 2>&1 || true
}
