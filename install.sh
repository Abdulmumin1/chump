...
if ! curl -fSL --progress-bar -o "$INSTALL_DIR/chump" "$url"; then
    echo -e "${RED}Download failed. Check if the release exists: ${url}${NC}"
    exit 1
fi

chmod +x "$INSTALL_DIR/chump"
